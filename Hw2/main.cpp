...
void work();//homework function

//draw ppap in mirror
void mirrorppap(float tanx, float tany, float tanz, float an, float rotatex, float rotatey, float rotatez, float scalex, float scaley, float scalez);
...
void mirrorppap(float tanx, float tany, float tanz , float an,float rotatex, float rotatey, float rotatez, float scalex, float scaley, float scalez) {
	...
	glPushMatrix();
	glTranslatef(tanx, tany, tanz);
	glRotatef(an, rotatex, rotatey, rotatez);
...
void work() {
	...
	for (int i = 0; i < 6; i++)
	{
		...
		glClear(GL_DEPTH_BUFFER_BIT);// 好讓鏡中ppap可以照順序畫出
		glDepthFunc(GL_LESS);
		
		if (i == 0) {
			mirrorppap(0,6,0,0,0,0,0,1,-1,1);
		}
		if (i == 1) {
			mirrorppap(0, -6, 0, 0, 0, 0, 0, 1, -1, 1);
		}
		if (i == 2) {
			mirrorppap(6, 0, 0, 0, 0, 0, 0, -1, 1, 1);
		}
		if (i == 3) {
			mirrorppap(-2, 0, -2, 90, 0, 1, 0, 1, 1, -1);
		}
		if (i == 4) {
			mirrorppap(2, 0, -2, -90, 0, 1, 0, 1, 1, -1);
		}
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("OpenGL Assignment 2-0556638"); //SUDO
